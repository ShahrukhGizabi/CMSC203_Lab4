   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   ...
	   int placeholder = column;
	   int MinimumRow = 0;

	  for(int row = 0; row < intArray.length; row++) {
	
		  if (intArray[MinimumRow][placeholder] > intArray[row][placeholder]) {
			   
			   MinimumRow = row;
			   
		   }
		  
		   animate(row, placeholder,intArray[row][placeholder] ); 
      ...
      return intArray[MinimumRow][column]; // replace this line with your return statement
